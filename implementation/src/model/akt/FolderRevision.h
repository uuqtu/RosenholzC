...
public:
    ...
    // Exactly one revision per folderId holds superseded = false
    ...
    // ------------------------------
    // latestRevNumber
    // Returns the highest rev number for a folderId, or 0 if none.
    // ------------------------------
    static uint32_t latestRevNumber(const std::string& folderId);
