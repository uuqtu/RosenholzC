#pragma once
// ============================================================
// F24.h  —  Step inside an F18Operation
//
...
public:
    ...
    // ── Status & result ───────────────────────────────────────
    F24StepStatus status { F24StepStatus::PENDING };
    ...
    F24Symbol displaySymbol() const;
    bool isComplete() const {
        return status == F24StepStatus::DONE || status == F24StepStatus::SKIPPED;
    }
    ...
    bool canStart(const std::vector<F24>& allSteps) const;
    ...
    // ── Factory ───────────────────────────────────────────────
    static std::shared_ptr<F24> loadById(const std::string& id);
    static std::vector<F24>     loadForVorgang(const std::string& operationId);
