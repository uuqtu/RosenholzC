// ============================================================
// F24.cpp  —  Step inside an F18Operation
// ============================================================
#include "F24.h"
...
void F24::fromRow(const Row& r) {
    ...
    status            = f24StepStatusFrom(g("status"));
...
bool F24::save() const {
    ...
    return d->exec(R"SQL(
        INSERT OR REPLACE INTO f24_steps (
        ...
        BindParam::text(f24StepStatusToString(status)),
...
bool F24::remove() const {
    auto* d = db(); if (!d) return false;
    return d->exec("DELETE FROM f24_steps WHERE step_id=?;",
                   {BindParam::text(stepId)});
}

// ── computeTrackingStatus ────────────────────────────────────
void F24::computeTrackingStatus() {
    if (status == F24StepStatus::DONE
    || status == F24StepStatus::SKIPPED) {
...
// ── canStart ─────────────────────────────────────────────────
bool F24::canStart(const std::vector<F24>& allSteps) const {
    // Terminal states can never be restarted
    if (status == F24StepStatus::DONE
    || status == F24StepStatus::SKIPPED) return false;
...
// ── Loaders ──────────────────────────────────────────────────
std::shared_ptr<F24> F24::loadById(const std::string& id) {
    auto* d = db();
    if (!d) return nullptr;
    auto rows = d->query("SELECT * FROM f24_steps WHERE step_id=?;",
                         {BindParam::text(id)});
    if (rows.empty()) return nullptr;
    auto s = std::make_shared<F24>();
    s->fromRow(rows[0]);
    return s;
}

std::vector<F24> F24::loadForVorgang(const std::string& operationId) {
    auto* d = db();
    std::vector<F24> result;
    if (!d) return result;
    auto rows = d->query(
        "SELECT * FROM f24_steps WHERE operation_id=? ORDER BY sequence_order,created_at;",
        {BindParam::text(operationId)});
    for (auto& r : rows) {
        F24 s; s.fromRow(r); result.push_back(s);
    }
    return result;
}

F24Symbol F24::displaySymbol() const {
    switch (status) {
        case F24StepStatus::DONE:
        case F24StepStatus::APPROVED:    return F24Symbol::DONE;
        case F24StepStatus::SKIPPED:
        case F24StepStatus::REJECTED:    return F24Symbol::SKIPPED;
        case F24StepStatus::IN_PROGRESS: return F24Symbol::IN_PROGRESS;
        case F24StepStatus::PENDING:     return F24Symbol::PENDING;
        case F24StepStatus::WAITING: return F24Symbol::WAITING;
        case F24StepStatus::BLOCKED: return F24Symbol::BLOCKED;
    }
    return F24Symbol::PENDING;
}

bool F24::complete() {
    if (isComplete()) return true;
    status        = F24StepStatus::DONE;
...
bool F24::update() const {
    auto* d = db(); if (!d) return false;
    // Refresh updatedAt via const_cast for save:  
    const_cast<F24*>(this)->updatedAt = nowIso();
    return save();
}
