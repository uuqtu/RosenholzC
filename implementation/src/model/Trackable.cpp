...
bool TrackableItem::save() const {
    ...
    bool ok = db->exec(R"(
        ...
        textOrNull(parentTrackableId),
        BindParam::text(title),
        textOrNull(description),
        BindParam::text(statusStr),
        textOrNull(priority),
        textOrNull(focusDate),
        textOrNull(dueDate),
        textOrNull(archivedDate),
        textOrNull(plannedDate),
        textOrNull(assigneeId),
        textOrNull(createdBy),
    ...
    for (const auto& r : reminders) {
        db->exec(R"(
            ...
            textOrNull(r.assigneeId), BindParam::text(r.title),
            textOrNull(r.message),    BindParam::text(r.remindAt),
            BindParam::int64(r.isRecurring ? 1 : 0),
            textOrNull(r.recurrenceRule),
            textOrNull(r.lastTriggered), textOrNull(r.nextTrigger),
            BindParam::int64(r.isDismissed ? 1 : 0), textOrNull(r.dismissedAt),
