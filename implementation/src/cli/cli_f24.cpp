...
//
// Public functions:
//   cmdF24(args)                — dispatch für 'rh -f18 ...'
...
static void drawF18Chain(const std::vector<Rosenholz::F24>& steps) {
...
void stepMenu(Rosenholz::F24& step,
    ...
    // F24 context: command-line driven, no number menu.
    ...
    auto printStepHeader = [&]() {
        ...
        std::cout << "\n  " << Color::bold(crumb) << "\n"
                  ...
                  << "  Titel     : " << step.title << "\n"
                  << "  Typ       : " << step.stepType << "\n"
                  << "  Status    : " << f24StepStatusToString(step.status) << "\n"
        ...
        auto koms = Communication::loadForOwner(step.stepId, "f24");
    ...
    while (true) {
        ...
        // ── f99 Note ──────────────────────────────────────────────────────
            ...
            if (!text.empty()) {
                auto n = Note::create("f24", step.stepId, text);
        ...
        // ── KOM ───────────────────────────────────────────────────────────
            ...
            std::string sub = args.empty() ? "-o" : args[0];
            if (sub == "-n") {
                communicationMenu(step.stepId, "f24");
            } else if (sub == "-o") {
                listComms(step.stepId, "f24");
                auto koms = Communication::loadForOwner(step.stepId, "f24");
                if (!koms.empty()) {
...
void printF18Operation(const Rosenholz::F18Operation& v) {
    ...
    if (!v.steps.empty()) {
        ...
        for (auto& s : v.steps)
            std::cout << "    [" << f24StepStatusToString(s.status) << "] " << s.title.substr(0,40) << "\n";
    }
}
...
static bool f18_step_list(std::shared_ptr<F18Operation> v) {
    ...
    for (auto& s : v->steps)
        std::cout << "  " << std::setw(3) << n++ << ". "
                  << "[" << std::left << std::setw(8) << std::string(Rosenholz::f24StepStatusToString(s.status)).substr(0,7) << "] "
...
// ── cmdF18s: F18 Step (F24) management ──────────────────────────────────────
...
void cmdF24Step(const std::vector<std::string>& args,
    ...
    if (!args.empty() && args[0] == "-e") {
        ...
        if (s.update()) printOk("F24 aktualisiert.");
        else            printErr("Fehler beim Speichern.");
        return;
    }
    ...
    if (!args.empty() && (args[0] == "-o" || args[0] == "-so")) {
        ...
        for (int i=0; i<(int)hits.size(); i++)
            std::cout << "  " << std::setw(4) << (i+1)
                      ...
                      << std::setw(12) << f24StepStatusToString(hits[i]->status)
                      << std::setw(28) << hits[i]->title.substr(0,26)
                      << hits[i]->stepType << "\n";
        if (!doSearch) return;
        int pick = readInt("  Auswahl [0=Abbrechen]", 0, (int)hits.size());
    ...
    if (v->steps.empty()) {
        std::cout << "  (keine F24)\n"
                  << "  f24 -n    neuen Schritt anlegen\n";
        return;
    }
    std::cout << "\n  F24-Schritte fuer " << Color::magenta(v->operationId)
              << " — " << v->title << ":\n"
              << "  " << std::left << std::setw(4) << "#"
              << std::setw(26) << "ID"
              << std::setw(12) << "STATUS"
    ...
    for (int i=0; i<(int)v->steps.size(); i++) {
        ...
        std::cout << "  " << std::setw(4) << (i+1)
                  ...
                  << std::setw(12) << f24StepStatusToString(s.status)
    ...
    std::cout << "\n  f18s <n>  oeffnen  |  f24 -n  neu  |  f18s -e <n>  bearbeiten\n\n";
